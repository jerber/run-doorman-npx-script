...
const sendUpdateToDoormanServer = async body => {
	...
	if (errorCode) {
		printToTerminal(
			"There was an error that calls for a Doorman engineer. Please contact jeremy@basement.social for help!"
		);
...
const hasMostRecentFirebseCliVersions = () => {
	...
	let {
		...
		"npm list -g firebase-admin && npm list -g firebase-tools && npm list -g firebase-functions",
		{ silent: true }
	);
	...
	for (pkg_str in pkg_strs) {
		if (!stdout.includes(pkg_str)) {
			printToTerminal(
				`The library ${pkg_str} is not up to date, so will install latest...`
			);
			return false;
		}
	}
...
const installFirebaseCLI = async () => {
	...
	if (mostRecent) {
		...
		shell.exec(
			"npm install -g firebase-tools && npm install -g firebase-functions@latest firebase-admin@latest --save"
		);
...
const downloadDoormanDownloadGit = async () => {
	...
	shell.exec(
		`git clone https://github.com/jerber/DoormanDownload.git ${DOORMAN_DIRECTORY}`
	);
...
const setConfigAndDeployFunction = async token => {
	...
	let { stdout, stderr } = shell.exec(
		`cd functions && firebase functions:config:set doorman.apisecret="${API_SECRET}" --token "${token}"`
	);

	const errorStr =
		"Authorization failed. This account is missing the following required permissions on project";
	const errorMessage =
		"Authorization failed! Are you sure you logged into the correct Firebase account?";
	...
	({ stdout, stderr } = shell.exec(
		`cd functions && firebase deploy --token "${token}" --only functions:doormanPhoneLogic`
	));
	...
	message = "Deployment over, now will see if it was successful...";
	printToTerminal(message);
...
const parseDeploymentResponse = async deploymentResposne => {
	...
	let location = deploymentResposne.substring(
		deploymentResposne.lastIndexOf("doormanPhoneLogic(")
	);
	...
	if (location !== "us-central1")
		console.log("***REGIION IS NOT USE CENTRAL***");
...
const startCLI = async () => {
	...
	printToTerminal(
		"DONE! Check your Doorman dashboard to see if the deployment was successful!"
	);
};
